...
class MainViewcontroller: UIViewController{
    ...
    var arrFoodCountry : [FoodCountry] = []
...
extension MainViewcontroller: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return arrFoodCountry.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! customCell
        cell.label.text = arrFoodCountry[indexPath.row].Country
        //
        
        let urlhinh = Config.serverURL + "/upload/" + arrFoodCountry[indexPath.row].Image
...
class customCell: UICollectionViewCell{
    ...
    override func layoutSubviews() {
        ...
        image2.snp.makeConstraints({(make) in
            ...
            image2.addGestureRecognizer(tapgs)
        })
        label.snp.makeConstraints({(make) in
            make.top.equalTo(image2.snp.bottom).offset(2)
